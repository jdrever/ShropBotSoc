<?php 
//Displays a summary table of records for an individual apsecies
...
echo '<h2>Displaying records for <i>'. $species['sppName']. '</i> (' . $species['sppCommon'] . ') - '.$total_rows. ' total</h2>';
echo '<div class="center">'.anchor('shrops/tetradmap/species/'.$species['sppid'],'Show Tetrad Distribution Map', 'title="Show Tetrad Distribution Map"').'</div>';
echo "<div class='center' id ='pagination'>" . $links . "</div>";

$this->table->set_template($tmpl);
$this->table->set_heading('','Site Name','Grid Ref','Tetrad','Recorder(s)','Year');
//Test Table extension
//$this->table->set_columns_to_hide(array('Recorder'));
